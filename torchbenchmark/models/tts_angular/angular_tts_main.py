    ...
    def _train(self, model, criterion, optimizer, scheduler, ap, global_step, c, niter):
        ...
        for reps in range(niter):
            for _, data in enumerate(self.SYNTHETIC_DATA):
                ...
                # print(outputs.shape)
                view = outputs.view(
                    c.num_speakers_in_batch,
                    outputs.shape[0] // c.num_speakers_in_batch,
                    -1,
                )
                # loss computation
                loss = criterion(view)
                ...
                avg_loss = (
                    0.01 * loss.item() + 0.99 * avg_loss
                    if avg_loss != 0
                    else loss.item()
                )
                avg_loader_time = (
                    1 / c.num_loader_workers * loader_time
                    + (c.num_loader_workers - 1)
